/**
 * AI SETTINGS
 * The free built-in AI is the default: it runs on this device with no
...
export function AiSettings({ api, onClose, onSaved }: { api: AiSettingsApi; onClose: () => void; onSaved: (v: AiSettingsView) => void }) {
  ...
  React.useEffect(() => {
    api.get().then((v) => {
      ...
      setShowOnline(v.provider !== "builtin");
    }).catch(() => undefined);
  ...
  const save = async (): Promise<AiSettingsView | null> => {
    ...
    try {
      ...
      if (v.provider === "builtin") {
        ...
        setStatus({ ok: true, message: "Saved. You're using the free built-in AI." });
      } else {
  ...
  const download = async () => {
    ...
    try {
      ...
      setDownloaded(true);
      setStatus({ ok: true, message: "The built-in AI is ready. It will work offline from now on." });
    } catch (e) {
      setStatus({ ok: false, message: e instanceof Error ? e.message : String(e) });
    } finally {
      setWorking("");
      ...
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="ai-title">
        <div className="modal-head">
          <div>
            ...
            <p>The built-in AI is free and needs no account. It runs right here on your device.</p>
          </div>
        ...
        <button role="radio" aria-checked={isBuiltin} className={`provider big${isBuiltin ? " on" : ""}`} onClick={() => choose("builtin")}>
          <span className="prov-top">
            <span className="prov-name">Built-in AI</span>
        ...
        {isBuiltin && (
          <div className="modal-form">
            <div className="size-grid" role="radiogroup" aria-label="Built-in AI size">
            ...
            <p className="modal-note">
              The model runs on your graphics chip using WebGPU (current Chrome, Edge, or Safari on macOS 26). If Chrome's own built-in AI is already set up on this computer, the app uses that instead and nothing is downloaded.
            </p>
          </div>
        )}
        ...
        <div className="modal-foot">
          ...
          {isBuiltin ? (
            <>
              ...
              <button className="btn primary" disabled={working !== "" || !unsaved} onClick={save}>{working === "save" ? "Saving…" : unsaved ? "Use built-in AI" : "Saved"}</button>
            </>
          ) : (
            <>
