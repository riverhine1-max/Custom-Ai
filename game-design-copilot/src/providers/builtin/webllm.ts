 ...
 *
 * The heavy WebLLM library is only loaded when the built-in AI is first used.
 */
...
export class WebLlmEngine {
  ...
  static async create(size: BuiltInSize, f16: boolean, workerUrl: string | null, onProgress: (p: Progress) => void): Promise<WebLlmEngine> {
    ...
    try {
      ...
      if ((e as Error)?.message !== "no worker") console.warn("[built-in AI] worker failed, running on the page instead", e);
  ...
  private async run(req: GenerateRequest): Promise<string> {
    ...
    try {
      ...
      throw new Error(`The built-in AI had a problem: ${(e as Error)?.message ?? e}`);
    } finally {
