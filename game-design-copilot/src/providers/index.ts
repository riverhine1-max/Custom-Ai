 ...
 * in the browser: the built-in AI on the device, the online services
...
export function providerFromConfig(c: AiConfig, deps: ProviderDeps = {}): ModelProvider {
  ...
  switch (c.provider) {
    ...
    case "gemini":
      if (!c.apiKey) return new OfflineProvider("Add your free Gemini key in AI settings, or switch back to the built-in AI.");
    ...
    case "openai":
      if (!c.apiKey) return new OfflineProvider("Add your OpenAI API key in AI settings, or switch back to the built-in AI.");
      return new OpenAIProvider({ apiKey: c.apiKey, model, label: "OpenAI", openaiNative: true });
    case "anthropic":
      if (!c.apiKey) return new OfflineProvider("Add your Anthropic API key in AI settings, or switch back to the built-in AI.");
...
export function viewOf(c: AiConfig): AiSettingsView {
  ...
  return {
    ...
    label: c.provider === "builtin" ? `Built-in AI · ${findBuiltIn(size).label}` : `${preset?.name ?? "AI"}${model ? ` · ${model}` : ""}`,
  };
}
