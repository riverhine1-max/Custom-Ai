...
// Installable-app files: manifest, icons, and the offline service worker (stamped per build).
fs.copyFileSync("web/manifest.webmanifest", `${outdir}/manifest.webmanifest`);
fs.cpSync("web/icons", `${outdir}/icons`, { recursive: true, filter: (f) => !f.endsWith("maskable.svg") });
fs.writeFileSync(`${outdir}/sw.js`, fs.readFileSync("web/sw.js", "utf8").replace("__BUILD__", String(Date.now())));
