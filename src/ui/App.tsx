...
import type { AiSettingsView, ProviderId } from "../providers/presets";
...
import { AppMark, ConfirmButton, Icons, timeAgo } from "./bits";
import { nameFromIdea } from "./text";

export interface Host {
  /** Web app: the AI settings screen (and Ludomuse controls). */
  ai?: AiSettingsApi;
  ...
  backups?: boolean;
  /** Installing the web app on this device (omitted where it can't be installed). */
  install?: InstallApi;
}

export type InstallState = "available" | "installed" | "manual" | "none";
export interface InstallApi {
  state(): InstallState;
  subscribe(fn: (s: InstallState) => void): () => void;
  /** Shows the browser's install prompt. */
  prompt(): Promise<void>;
  /** How to install by hand where there's no prompt (Safari). */
  manualHint: string;
}
...
export function App({ client, host }: { client: CopilotClient; host: Host }) {
  ...
  const [aiStart, setAiStart] = React.useState<ProviderId | undefined>();
  const [welcome, setWelcome] = React.useState(() => !!host.ai && host.prefs?.get("gdc.welcome.v1") !== "1");
  const [install, setInstall] = React.useState<InstallState>(() => host.install?.state() ?? "none");
  ...
  React.useEffect(() => host.install?.subscribe(setInstall), [host.install]);

  const pickAi = async (id: ProviderId) => {
    host.prefs?.set("gdc.welcome.v1", "1");
    setWelcome(false);
    if (id === "builtin") {
      try {
        setAi(await host.ai!.save({ provider: "builtin", size: ai?.size ?? "balanced" }));
        toast("You're using Ludomuse. Type your game idea to begin.");
      } catch (e) {
  ...
  // The AI in use, and Ludomuse's download status.
  React.useEffect(() => {
  ...
  const setupBanner =
    host.ai && usingBuiltin && !aiBanner ? (
      phase === "checking" || phase === "downloading" || phase === "loading" ? (
        ...
        <div className="ai-banner">
          <div>
            <strong>Ludomuse, your free AI, runs on this device.</strong> No account needed. The first time, it downloads once ({findBuiltIn(ai?.size).download}); after that it starts in seconds and works offline.
          </div>
          <div className="row">
  ...
  const aiName = ai ? (ai.provider === "builtin" ? `Ludomuse · ${phase === "ready" ? "ready" : downloaded ? "downloaded" : findBuiltIn(ai.size).label}` : ai.label) : "…";
    ...
    <div className="shell">
      ...
      <aside className={`sidebar${sideOpen ? " open" : ""}`} aria-label="Your games">
        ...
        <div className="side-foot">
          ...
          {host.install && install !== "installed" && install !== "none" && (
            <button
              className="btn xs install-btn"
              onClick={() => (install === "available" ? host.install!.prompt() : toast(host.install!.manualHint))}
              title="Install Game Design Copilot as an app on this device"
            >
      ...
      {aiOpen && host.ai && (
        <AiSettings
          api={host.ai}
          initialProvider={aiStart}
          onClose={() => { setAiOpen(false); setAiStart(undefined); }}
          onSaved={(v) => { setAi(v); toast("AI settings saved."); }}
        />
      )}
      {welcome && host.ai && <Welcome onPick={pickAi} />}
