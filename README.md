...
**It's free and works on its own.** Its AI, **Ludomuse**, runs inside your web browser on
your own device. There's no account, no sign-in, no API key and no subscription. Your
games are saved on your device too. When you first open it, you choose Ludomuse,
ChatGPT or Claude.

**It's an app.** Install it from the browser (the **Install app** button, or Chrome's
install icon in the address bar) and it gets its own window and icon and works offline.
There's also a Windows desktop version you can download and double-click.

**Ludomuse** is the copilot's own game-design AI: the app's design rules and memory
running on an open model (Qwen 3.5) on your graphics chip, or on Chrome's built-in
Gemini Nano when that's ready. It isn't a model trained from scratch.
...
- **Chrome or Edge (Windows, Mac, ChromeOS, Linux):** open the app, then click
  **Install app** in the sidebar (or the install icon at the right of the address bar).
- **Safari on Mac:** File → Add to Dock. **iPhone / iPad:** Share → Add to Home Screen.
- **Windows desktop download:** `npm run desktop:win` builds
  `release/Game-Design-Copilot-Windows.zip`. Unzip it and double-click
  `Game Design Copilot.exe`. Windows SmartScreen may warn because the app isn't
  code-signed: click **More info → Run anyway**. It's the same app in an Electron window
  (`desktop/main.cjs`), with its files stored locally.

After installing, the app opens offline (the service worker in `web/sw.js` keeps its files).

### Publishing the Windows download (automatic)

This repo includes `.github/workflows/windows-app.yml`. Every push to `main` makes
GitHub build the Windows app and put it on the repo's **Releases** page, so the
download link is always `https://github.com/<you>/<repo>/releases/latest`. To release a
new version, change `"version"` in `package.json` and push.

## Extra mechanics

- **Design Dice** (the **Dice** button by the message box): rolls a random "what if…"
  twist or limit and writes a prompt asking whether it fits your main goals. You decide
  whether to send it.
- **Game size meter** (in the Summary panel): counts your systems, mechanics and content
  to show Tiny → Huge, plus how many of the 12 basics are decided. Catches scope creep
  early.
- **A loading screen** and a **welcome screen** where you pick your AI.
...
```
web/                    ← the web app: page + loading screen, startup (main.tsx), AI worker,
                          app manifest, offline service worker (sw.js), icons/
desktop/                ← the Windows desktop app (Electron main.cjs, icon)
src/
  core/                 ← all the design logic. No framework, no AI library. Start here.
    ...
    extras.ts           ← Design Dice and the game size meter
...
```bash
npm test            # 68 tests: safety rules, memory, saving, built-in AI, full scripted sessions
npm run typecheck
```
